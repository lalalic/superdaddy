var {React,Component,UI,Router}=require('dashboard'),
    {List, Loading, Empty,Comment,CommandBar,Photo}=UI,
...
export default class Dashboard extends Component{
    ...
    _getQuery(when){
        ...
        if(delta>0)
            return (<QueryPast title={delta==1 ? "Yesterday" : when} when={when}/>)
        else if(delta<0)
            return (<QueryFuture title={delta==-1 ? "Tomorrow" : when} when={when}/>)
        else
    ...
    onChangeDate(d){
        ...
        this.context.router.transitionTo("dashboard",{when:f})
...
class TaskQueryCommand extends DialogCommand{
    renderContent(){
        var {when=new Date(), onChange}=this.props
        return [(<div style={{}}>Approvings</div>),
                (<div>
                    <Calendar
                        displayDate={when}
                        minDate={when}
                        maxDate={Date.Helper.addDays(when,31)}
