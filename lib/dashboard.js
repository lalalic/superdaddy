        ...
        return (
            <div>
                <Empty text="Start from your first baby!"
                    icon={<IconBaby onClick={()=>this.context.router.transitionTo("baby")}/>}/>
                <CommandBar
                    className="footbar"
                    primary="Family"
    ...
    renderContent(when){
        ...
        if('approvings'==when){
            Object.assign(props,{
                ...
                empty:"Nobody played with your children!"
            })
        }else{
            ...
            Object.assign(props,{
                ...
                template:TaskItem,
                empty:d>0 ? `You didnot play with ${child.name}` :
                    (<Empty text={`Find fun topic for ${child.name}`}
                        icon={<IconKnowledges onTouchTap={()=>this.context.router.transitionTo('knowledges')}/>} />)
            })
        }
