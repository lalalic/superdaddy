var {React,Component,UI:{List, CommandBar, selectFile, Empty}}=require('qili-app'),
...
class Item extends Component{
    render(){
        ...
        return (
            <div className="li inset" {...others} onClick={this.onDetail.bind(this)}>
                <h4>{model.title}</h4>
                <div className="photo">
                    <img src={model.photo||image}/>
                </div>
                <div><div>{model.createdAt||model.updatedAt}</div><div>赞{model.zans}</div></div>
            </div>
        )
    }
    onDetail(){
