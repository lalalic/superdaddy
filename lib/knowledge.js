var {React,Component,UI:{List,Loading,Comment,CommandBar,selectFile},Router:{Link}, User}=require('dashboard'),
...
export default class Knowledge extends Component{
    ...
    onSelect(command){
        ...
        case 'New Version':
            Knowledge.selectFile()
                .then((docx)=>{
                    extractor.invokeImageUrls('div.content')
                    var pending=Object.assign({},this.state.entity,docx)
        ...
        case 'Save':
            dbKnowledge.upsert(this.state.entity, this.origin,
                (updated)=>this.setState({entity:updated,status:undefined}))
            break
        }
    }
    ...
    static selectFile(){
        return selectFile('raw')
            .then(extractor)
            .then((docx)=>{
                var {name,title, keywords, category, subject, abstract,description, content, ...others}=docx
                return {content,title:title||name,summary:abstract||description||subject,keywords,category,props:others}
            }, console.error)
    }
}
