...
export const ACTION={
	ADD: knowledge=>(dispatch, getState)=>{//core/index.js ACTION.RESET cloned this logic
		let {_id,title,score, days, fields}=knowledge
		let task={knowledge:_id,content:title,score, days, fields}
		let ps=[]
...
export const TimeManage=({id, manageMyTime,minHeight})=>(
    <div>
		<div style={{minHeight}}>
			<BabyTimeManage/>
		</div>
		{
			(function(){
				...
				if(!MyTimeManage)
					MyTimeManage=TimeManageCreator(MyAppBar,id)
